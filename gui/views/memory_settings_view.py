...
class MemorySettingsView(QWidget):
    ...
    def __init__(self, memory_vm: Any):
        ...
        for label, value in [("全部", ""), ("pending", "pending"), ("accepted", "accepted"), ("rejected", "rejected"), ("expired", "expired")]:
    ...
    def render(self, model: Dict[str, Any]) -> None:
        ...
        self._render_candidates(model.get("candidates") or [])
        self._render_memories(model.get("memories") or [])
    ...
    def load_memories(self) -> None:
        self.render(self.memory_vm.load_memories())
    ...
    def _preview_card(self) -> Card:
        ...
        card.add_body_widget(self.refresh_previews_button)
    ...
    def _render_candidates(self, rows: list[Dict[str, Any]]) -> None:
        ...
        if not rows:
            self.candidates_list.addItem("尚未加载或没有 MemoryCandidate。")

    def _render_memories(self, rows: list[Dict[str, Any]]) -> None:
        ...
        if not rows:
            self.memories_list.addItem("尚未加载或没有 SavedMemory。")
