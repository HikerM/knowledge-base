...
- v2.0.0-beta.6 workspace creation wizard 是 plan-only 阶段；GUI 只能通过 `WorkspaceCreationPlanService` 生成 dry-run 计划和预览，不得实现真实创建逻辑，不得绕过 service layer，不得自动创建 workspace，不得自动创建 `.kb/index.sqlite`，不得自动 index。
- 未来创建 workspace 必须先走 `WorkspaceCreationPlanService` 生成 plan；`WorkspaceCreateService` 只能在用户确认 plan 或 plan hash 后写入 `workspace.yaml` 和目录结构，且当前阶段不得实现。
- 新建 workspace 不得自动导入旧资料，不得创建正式知识，不得依赖 Git，不得把软件安装目录或当前工作目录默认当作 workspace。
