...
- v2.0.0-beta.5 workspace creation wizard 仍是设计阶段；不得实现真实创建逻辑，不得自动创建 workspace，不得自动创建 `.kb/index.sqlite`，不得自动 index。
- 未来创建 workspace 必须先走 `WorkspaceCreationPlanService` 生成 plan；`WorkspaceCreateService` 只能在用户确认 plan 或 plan hash 后写入 `workspace.yaml` 和目录结构。
