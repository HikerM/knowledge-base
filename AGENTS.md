...
- v2.4.0 只允许设计 ConversationStore / MemoryService、memory algorithm review 和 assistant evaluation plan；不得实现真实持久化 memory service，不得保存真实长期记忆，不得实现 conversation store 写入，不得接真实 AI、OpenAI、本地模型、ModelScope、模型下载、RSS/vector 或 mutation UI。
- v2.4.1 只允许实现 ConversationStore / MemoryService / retention static models 和 schema validation；模型必须保持 IO-free，只能做 `to_dict()` / `from_dict()` / `validate()`，不得实现真实 ConversationStore 写入、不得实现持久化 MemoryService、不得保存真实长期记忆、不得创建 `workspace/ai`、不得接真实 AI、OpenAI、本地模型、ModelScope、模型下载、RSS/vector 或 mutation UI。
- v2.4.2 只允许实现 ConversationStore / MemoryService in-memory service harness 和 strict schema hardening；harness 只能使用进程内 dict/list，不得写文件、不得创建 `workspace/ai`、不得写 conversation 文件、不得把 memory 保存到磁盘、不得进入 formal search、不得授权 mutation、不得接真实 AI、OpenAI、本地模型、ModelScope、模型下载、RSS/vector 或 mutation UI。
- v2.5.0 只允许设计 Persistent ConversationStore / MemoryService storage，不得实现真实持久化，不得创建 `workspace/ai`，不得创建 conversation 文件，不得保存长期记忆到磁盘，不得接真实 AI、OpenAI、本地模型、ModelScope、模型下载、RSS/vector 或 mutation UI，不得修改 `knowledge/**/*.md`、SQLite schema 或 search/index/audit 行为。
- v2.5.1 只允许实现 Persistent storage static contract tests，把 v2.5.0 边界转成静态模型、plan 模型和纯函数 validator；不得实现真实持久化，不得创建 `workspace/ai`，不得写 conversation/memory/draft 文件，不得保存长期记忆到磁盘，不得接真实 AI、OpenAI、本地模型、ModelScope、模型下载、RSS/vector 或 mutation UI，不得执行 mutation，不得修改 `knowledge/**/*.md`、SQLite schema 或 search/index/audit 行为。
- v2.5.2 只允许实现 AI persistence minimal bootstrap 和 atomic JSON writer；bootstrap 必须由 service layer 显式调用且 `confirmed=True`，只能创建 `workspace/ai/`、`workspace/ai/conversations/`、`workspace/ai/memory/`、`workspace/ai/drafts/`、`workspace/ai/indexes/` 和 `workspace/ai/manifest.json`。
- v2.5.2 不得保存 conversation / memory 正文，不得创建 `conversations/*.jsonl`、`memory/*.jsonl` 或 draft records，不得保存长期记忆到磁盘，不得接真实 AI、OpenAI、本地模型、ModelScope、模型下载、RSS/vector 或 mutation UI，不得修改 `knowledge/**/*.md`、SQLite schema 或 search/index/audit 行为。
- v2.5.3 只允许实现 Conversation persistence minimal implementation；只能通过 `ConversationPersistenceService` 显式读写 `workspace/ai/conversations/manifest.json`、`workspace/ai/conversations/conv_<id>/conversation.json` 和 `workspace/ai/conversations/conv_<id>/messages.jsonl`。
- v2.5.3 conversation persistence 必须先验证 `workspace/ai/manifest.json` 已存在且有效；不得自动 bootstrap，不得 startup scan，不得 GUI/provider/ViewModel 直接 IO，不得自动加载全部 conversation，不得进入 formal search，不得写 `.kb/`、`knowledge/` 或 SQLite。
- v2.5.3 conversation persistence 必须包含 recovery hardening；`append_message` 必须在写入前完成 message、conversation metadata 和 manifest validation，并保持 `messages.jsonl`、`conversation.json`、`manifest.json` 跨文件一致；失败必须 controlled error 且不得静默留下 stale metadata。
- v2.5.3 `delete_conversation` 必须使用明确 staging/trash 语义；manifest update 失败必须恢复 active conversation dir，trash cleanup 失败可以保留 cleanup-pending trash，但 active manifest 不得继续列出已删除 conversation。
- v2.5.3 仍不得实现 MemoryService 持久化，不得写 `workspace/ai/memory/*.jsonl`，不得保存长期记忆到磁盘，不得接真实 AI、OpenAI、本地模型、ModelScope、模型下载、RSS/vector 或 mutation UI。
- v2.7.0 只允许设计 Local Model Installer，不得下载模型，不得接 ModelScope 实际下载，不得接 OpenAI，不得接本地 `llama.cpp` / server，不得启动模型进程，不得执行 shell 下载脚本，不得做真实 AI provider，不得修改 GUI 执行逻辑，不得修改 `knowledge/**/*.md`、SQLite schema 或 search/index/audit 行为。
- v2.7.1 只允许实现 Local Model Catalog static loader、schema validation 和 policy tests；不得下载模型，不得访问 ModelScope 网络，不得接 OpenAI，不得接本地模型 runtime，不得启动模型进程，不得接 `llama.cpp` / server，不得开放本地 HTTP，不得做真实 AI provider，不得修改 `knowledge/**/*.md`、SQLite schema 或 search/index/audit 行为。
- v2.7.2 只允许实现 Model Download plan-only TaskQueue design、`ModelDownloadPlan` schema 和 dry-run plan service；不得下载模型，不得访问 ModelScope 网络，不得写模型文件，不得创建模型目录，不得创建真实 TaskQueue task，不得接 OpenAI、本地 runtime、`llama.cpp` / server、本地 HTTP 或真实 AI provider，不得修改 `knowledge/**/*.md`、SQLite schema 或 search/index/audit 行为。
- model download 必须保持 plan-only，直到未来阶段明确允许真实执行；真实执行前必须有 user confirmation、TaskQueue、path safety、available disk check、checksum gate、progress/cancel/retry/cleanup policy 和 no-shell/no-arbitrary-command 静态测试。
- `sha256=pending` 只能生成 blocker，不能进入 verified install；checksum 通过前不得 register model，不得标记模型已安装。
- 未来 installer/runtime 接入前必须先通过 catalog validation；catalog 默认模型必须保持 `Qwen3-0.6B-GGUF Q4_K_M` ultra_light，30GB+ 模型不得作为默认、默认推荐或首次安装建议。
- local model catalog 必须保持 `provider_kind=local`、单文件 `.gguf`、`source_kind` 仅允许 `modelscope_reference` / `manual_reference`；`sha256=pending` 必须阻止 verified install，`verified_install_allowed=true` 时 sha256 不能是 pending。
- 本地模型安装助手必须不执行 shell scripts，不得允许 arbitrary command，不得把用户输入、catalog 字符串、model_id 或 filename 拼接成命令。
- 本地模型不得放 workspace，不得放安装目录；默认模型目录必须是 `%LOCALAPPDATA%\PersonalKnowledgeBase\models\` 或用户显式选择且通过 path safety 校验的目录。
- 本地模型 catalog 默认模型必须是 `Qwen3-0.6B-GGUF Q4_K_M` 或等价极轻量档；30GB+ 模型不得作为默认模型、默认推荐或首次安装建议。
- 所有未来模型下载都必须用户确认并通过 TaskQueue，必须有进度、可取消、错误详情、sha256 校验和 registry 记录；`sha256=pending` 时不得进入 verified install。
- 本地 model provider 未来仍必须遵守 AI control plane：`ContextBuilder` 只能通过 services，provider 不得直接读 Markdown / SQLite，不得绕过 CapabilityRegistry / PermissionPolicy，不得执行 mutation。
- AI persistence bootstrap 不得在 GUI 启动、workspace-status、App startup、adapter、provider 或 ViewModel 中自动执行；`plan_bootstrap` 只能 dry-run，真实 bootstrap 必须显式确认。
- conversation / memory 持久化必须作为独立阶段设计和审计，先补 storage layout、source of truth、schema version、atomic writes、deletion、retention、backup、export、privacy mode、secret/sensitive blocking、用户可见控制、migration 和回滚验收；不得从 in-memory harness 直接演进为未审核磁盘写入。
- AI persistence 未来只能放在 workspace-scoped `ai/conversations/`、`ai/memory/`、`ai/drafts/` 和可重建 `ai/indexes/`；不得放入 `knowledge/`、`.kb/` 或安装目录。
- AI persistence 的 JSONL / JSON records 才能作为 source of truth；SQLite 如用于 conversation/memory 查询只能是 derived index，可删除重建，不得进入 `.kb/index.sqlite`，不得进入 formal search。
- AI persistence 每个 record 必须有 `schema_version`，每个目录必须有 manifest；旧 schema 读入前必须 validate，未知 schema 必须生成 migration-required / blocked plan。
- AI persistence 所有文件 IO 必须由 service layer 拥有；GUI、AI provider、ViewModel、CLI wrapper、adapter 和静态 model 不得直接读写 conversation、memory、draft 或 AI derived index 文件。
- AI persistence bootstrap 之外，只有 v2.5.3 `ConversationPersistenceService` 可以显式保存 conversation metadata 和 message JSONL；MemoryService、draft、derived index、GUI/provider/ViewModel/CLI wrapper/adapter 直接 IO 仍是 future work 或 forbidden。
- AI persistence implementation 前必须先有 migration plan、rollback plan、local snapshot / backup、approval、TaskQueue、corruption recovery、retention cleanup、delete/clear/export 和 privacy mode tests。
- Actual AI persistence implementation cannot start until v2.5.1 persistence contract tests pass；persistence modules must remain IO-free unless a future explicitly scoped service layer owns the IO.
- App startup、workspace-status、formal search、index 和 audit 不得扫描 `ai/conversations/`、`ai/memory/` 或 `ai/drafts/`；AI data 不得注入 rules/checklists/snippets 搜索结果。
- AI backup 必须默认 `include_ai_conversations=false`、`include_ai_memory=false`、`include_ai_drafts=false`，并在 manifest 中记录显式 inclusion flags 和 privacy warning。
- AI conversation / memory schema validation 必须保持严格类型：bool 字段只能是真 bool，int 字段只能是真 int，list / dict 字段必须是真 list / dict，字符串 `"true"` / `"false"` 和 `0` / `1` 不得替代 bool，provider_kind / role / message type / memory status / sensitivity 必须严格枚举。
- conversation persistence 只能通过 v2.5.3 `ConversationPersistenceService` 显式 service API；memory persistence 仍是 future work，GUI、AI provider、ViewModel、CLI wrapper 和静态 model 不得直接读写 conversation、memory 或 draft 文件。后续 GUI conversation history viewer 只能依赖该 service 的显式分页读取，不得 startup auto-load。
- conversation / memory model 测试必须证明不会创建 `workspace/ai` 文件或目录。
...
- Memory candidate scoring 只能用于排序、去重和提示优先级；高分也不能自动保存长期记忆，用户确认是唯一保存条件。
- AI assistant 的 intent accuracy、capability safety、retrieval quality、memory quality、privacy、performance 和 golden test set 指标必须在实现持久化 ConversationStore、MemoryService 或真实 provider 前固化。
