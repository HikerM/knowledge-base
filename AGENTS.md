...
- v1.7.0 起点阶段只实现 TaskQueue baseline；`future_restore`、`future_archive`、`future_template_apply` 只能创建 task record，不得执行真实 restore/archive/template apply。
- destructive task 只能在 v1.8+ safe execute mutation 阶段接入，并且必须先满足 plan-only、local snapshot / backup、人工确认、错误详情、日志和可回滚要求。
