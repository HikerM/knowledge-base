...
- v1.8.0 Safe Execute Mutation Framework 只允许 `category_update_display_name_execute`，并且必须满足 plan、local snapshot / backup、approval、TaskQueue；只能修改 `config/categories.yaml` 的 `display_name`，不得 path rename、不得修改 Markdown、不得修改 SQLite schema。
...
- 当前 TaskQueue safe executable set 只允许执行 `noop`、`workspace_status`、`backup_create`、`audit`、`index`、`category_update_display_name_execute`；其中 category display_name 执行必须有 approval 且不得修改 Markdown 源数据。
