...
- AI persistence bootstrap 之外的实际 conversation / memory 持久化仍是 future work；GUI、AI provider、ViewModel、CLI wrapper 和 adapter 仍不得直接读写 AI persistence 文件。
...
- conversation / memory persistence 只能在未来 service layer 中实现；GUI、AI provider、ViewModel、CLI wrapper 和静态 model 不得直接读写 conversation、memory 或 draft 文件。
