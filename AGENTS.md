...
- workspace startup 只读当前 workspace `workspace.yaml` 和 `.kb/index.sqlite` metadata，不得启动时全量扫描知识库、全量读取 Markdown 或自动全量 index。
...
- App startup 不得扫描 `knowledge/`、不得读取所有 Markdown、不得自动全量 index。
- Category view 必须从 SQLite `documents` metadata 聚合统计；Search view 必须从 SQLite FTS5 查询。
...
- SQLite missing/stale 时，GUI 必须显示 index status，并提供后台 index/reindex 任务入口。
...
- 不得实现启动时自动全量 index。
- 启动、分类、搜索、review queue、archive/trash/quarantine 列表必须走 SQLite metadata / FTS5 热路径。
- Markdown 只在 open/edit/reindex/doctor/secret-scan/schema migration 等明确需要源文件的操作中读取。
- 不得让 GUI 一次渲染所有搜索结果。
