...
- v2.4.1 仍不接 OpenAI、本地模型、ModelScope，不下载模型，不做 RSS/vector，不实现真实 AI 问答，不实现真实 ConversationStore 写入，不实现持久化 MemoryService，不创建 `workspace/ai`，不保存真实长期记忆，不执行 mutation，不改变 search/index/audit 行为。
