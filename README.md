...
- Workspace Creation Wizard Design 见 [docs/workspace-creation-wizard-design.md](D:/AI/personal-knowledge-base/docs/workspace-creation-wizard-design.md)；v2.0.0-beta.6 已实现 plan-only 创建计划服务和 GUI 计划预览，但不实现真实创建逻辑。
...
- 新建知识库向导当前是 plan-only 阶段；GUI 只能通过 `WorkspaceCreationPlanService` 生成 dry-run 计划并展示预览，不得绕过 service layer，也不得直接写目标目录。
- 真实创建仍是未来工作；未来必须先通过 `WorkspaceCreationPlanService` 生成 plan，用户确认后才允许 `WorkspaceCreateService` 写入 `workspace.yaml` 和目录结构。
...
- v2.0.0-beta.6 的 workspace creation wizard 是 plan-only 阶段；`WorkspaceCreationPlanService` 只生成创建计划，不写文件、不创建 `.kb/index.sqlite`、不运行 index、不导入旧资料、不创建正式知识。
