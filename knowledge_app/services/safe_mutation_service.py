...
v1.8.0 intentionally exposes one low-risk mutation: updating a category
display_name in config/categories.yaml. Destructive operations remain
unsupported.
"""
...
SUPPORTED_MUTATION_PLAN_TYPE = "category_update_display_name"
SUPPORTED_MUTATION_TASK_TYPE = "category_update_display_name_execute"
...
class SafeMutationService:
    """Validate approvals and execute the v1.8.0 safe mutation subset."""
    ...
    def create_approval(
        ...
        if plan_result.plan_type != SUPPORTED_MUTATION_PLAN_TYPE:
    ...
    def execute_category_display_name_update(
        ...
        after_content = self._updated_categories_text(category_id, new_display_name, before_content)
        ...
        return MutationResult(
            success=True,
            mutation_type=SUPPORTED_MUTATION_TASK_TYPE,
            target={
    ...
    def _updated_categories_text(self, category_id: str, new_display_name: str, content: str) -> str:
        ...
        replacement = f"{' ' * field_indent}display_name: {self._yaml_string(new_display_name)}{newline}"
        for index in range(category_index + 1, end_index):
            line = lines[index]
            stripped = line.strip()
            indent = len(line) - len(line.lstrip(" "))
            if indent == field_indent and stripped.startswith("display_name:"):
