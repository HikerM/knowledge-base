...
- 运行时页面优先读取 SQLite metadata / FTS5；只有 open/edit/reindex/doctor 等明确操作读取 Markdown 源文件。
...
- 启动、分类页、搜索页、review queue、Archive / Trash / Quarantine 页面默认读取 SQLite metadata / FTS5。
- 搜索默认只查 SQLite FTS5 / 索引，不全量读取 Markdown。
- Category view 从 `documents` metadata 聚合 category/layer/status/review_required/confidence 统计。
- Review queue 从 `documents` metadata 查询待审核项。
- Archive / Trash / Quarantine 页面从 `documents` metadata 分页查询列表。
- `open` / `edit` 才根据 `documents.path` 读取单篇 Markdown 全文。
...
- SQLite missing/stale 时，GUI 显示 index status 和受限能力，并提供后台 index/reindex 任务。
